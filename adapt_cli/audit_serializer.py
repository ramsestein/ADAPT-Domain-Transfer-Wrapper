"""
adapt_cli.audit_serializer
============================
Serialización completa del pipeline ADAPT en formato YAML auditado.
...
- versions : ADAPT + dependencias críticas (numpy, sklearn, xgboost, scipy)
...
def _adapt_version() -> str:
    """Versión del propio paquete ADAPT."""
    try:
        return importlib.metadata.version("adapt")
    except Exception:
        try:
            from adapt import __version__  # type: ignore
...
def serialize_run_audit(
    ...
    # Config (sin paths sensibles, solo sección adapt + overfitting)
    safe_config = {}
    if config_dict:
        safe_config = {
            k: v for k, v in config_dict.items()
            if k in ("adapt", "overfitting_check", "regularization", "joint_drift")
        }
