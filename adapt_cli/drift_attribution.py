...
Dado el triangle (raw, adapted, oracle):
    ...
    auroc_adapted — modelo source después de ADAPT
...
-----------------------------
Para cada feature j:
    1. Construir pipeline ADAPT con feature j SIN alinear (resto sí)
    2. AUROC_j_unaligned
...
def feature_recovery_attribution(
    ...
    Para cada feature j activa en la pipeline ADAPT:
...
def _attribution_one_feature(
    ...
    if config.apply_pca_coral and auto_adapter._fitted_aligner is not None:
        ...
        if idx_corr_no_j:
            ...
            if k >= 1:
                from domain_transfer.align.pca_coral import PCACoralAligner as _PCA
