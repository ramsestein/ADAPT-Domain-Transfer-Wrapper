...
def run(cfg: FullConfig, run_cv: bool = True, skip_expensive: bool = False) -> dict:
    ...
    print(f"  After filter_target({cfg.adapt.max_missing_rate}): n_target={len(pair_filt.y_t)}")
    ...
    try:
        ...
        if cfg.joint_drift.compute_mi_matrix:
            from adapt_cli.joint_drift import mi_matrix_delta
    ...
    if m_source is not None:
        ...
        try:
            from adapt.profiler.global_profiler import _calibration_metrics as _cal_metrics
    ...
    if skip_expensive:
        ...
        if eval_cfg.feature_attribution and oracle_result is not None:
            try:
                from adapt_cli.drift_attribution import feature_recovery_attribution
        ...
        if eval_cfg.counterfactual_alternatives > 0:
            try:
                from adapt_cli.counterfactuals import run_counterfactual_sensitivity
        ...
        try:
            from adapt_cli.patient_profiles import analyze_patient_profiles
